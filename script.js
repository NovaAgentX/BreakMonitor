...
const API_URL = "https://script.google.com/macros/s/AKfycbx_ZdLPNBYKujV32vBunl0rBXViMCSI9gCvsRMaTyWeTrKgEbYpqEelY__Ph3GIbrP-Ug/exec";
...
function initDashboardPage() {
  ...
  if (isTeamLead) {
    ...
    if (resetPasswordPanel) resetPasswordPanel.style.display = "";
  }
  ...
  // ---- Team Lead: Reset a forgotten password for any user ----
  const adminResetForm = document.getElementById("adminResetForm");
  if (adminResetForm) {
    adminResetForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const username = document.getElementById("resetEmpUsername").value.trim();
      const newPassword = document.getElementById("resetEmpPassword").value;
      ...
      submitBtn.innerHTML = `<span class="spinner-dot"></span> Resetting...`;
      try {
        const res = await apiCall("adminResetPassword", { username, newPassword });
        if (res && res.success) {
          showToast(res.message || "Password reset.", "success");
          adminResetForm.reset();
        } else {
          errorEl.textContent = (res && res.error) || "Could not reset password.";
        }
      } catch (err) {
        errorEl.textContent = "Something went wrong. Please try again.";
      } finally {
